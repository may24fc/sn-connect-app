...
- **UNVERIFIED:** No dedicated Christmas-tree implementation was confirmed. The local `rg` executable is unavailable, so the planned discovery task must identify any overlapping event feature before implementation.
...
- [ ] **0.1** Identify the owner route under `apps/web/src/app/(employee)` and reuse the closest authenticated interactive feature's API, hook, and component patterns. **Effort:** S. **Requirement:** FR-01. **Skip risk:** Feature is built outside portal conventions and incurs integration rework. **MEDIUM**.
...
- [ ] **1.1** Add a timestamped migration for `christmas_tree_events`, `christmas_ornaments`, and `christmas_wishes`, including standard audit columns and foreign keys to authenticated users. Store three season windows and deadlines; constrain `asset_type` to the eight approved ball designs. **Effort:** M. **Requirement:** US-01, US-02, FR-02, FR-04. **Skip risk:** Invalid assets, orphaned data, and seasons that cannot be configured safely. **HIGH**.
- [ ] **1.2** Enforce one ornament per user per event with a unique database constraint. Model October's Top 3 as three ordered personal-wish entries (`category = personal`, `item_number = 1..3`), plus one November and one December category entry; prevent duplicates with a unique constraint. **Effort:** S. **Requirement:** US-01, US-02. **Skip risk:** Concurrent requests create duplicate ornaments or incomplete October submissions. **HIGH**.
- [ ] **1.3** Store positions as normalized percentages (`position_x`, `position_y`) with database bounds and reserve placement generation to the server. **Effort:** M. **Requirement:** FR-03. **Skip risk:** Ornaments render off-tree, overlap severely, or drift across viewports. **HIGH**.
- [ ] **1.4** Add RLS policies: authenticated users can read ornaments for the active event, create/update only their own ornament and wishes, and cannot change `user_id`, event, slot, or submission timestamps. Limit event configuration and moderation to approved admin roles. **Effort:** L. **Requirement:** FR-01, FR-04. **Skip risk:** A user can alter another teammate's ornament or bypass ownership. **CRITICAL**.
- [ ] **1.5 HEADS-UP** Enforce category release and 20th-of-month deadline rules in a database RPC or server-side transaction using database/server time, never the browser clock. The UI lock is presentation only; direct HTTP requests must receive `403` before release or after deadline. **Effort:** M. **Requirement:** US-02, FR-04. **Skip risk:** Users bypass time gates or deadlines by changing local time or replaying requests. **HIGH**.
- [ ] **1.6** Compute event decoration state from active participants: garland for complete October submissions, lights for complete November submissions, and top star for complete December submissions. Define active participants from eligible non-deleted, active employee/associate accounts. **Effort:** M. **Requirement:** FR-03, FR-04. **Skip risk:** Team milestones unlock inconsistently or cannot account for onboarding/offboarding. **HIGH**.
