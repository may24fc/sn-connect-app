...
- [x] **4.3 HEADS-UP** Confirm Realtime replication and RLS behavior in staging before enabling it in production. A subscription must never receive another user's locked or unauthorized wish payload, even briefly. **Effort:** M. **Requirement:** US-02, US-03. **Verified on hosted Dev September 16; 14/14 assertions passed.** `20260915000001` publishes both tables idempotently and pins `REPLICA IDENTITY DEFAULT`. With user B subscribed under its own token via `realtime.setAuth`, a locked `for_sn` row was force-updated by disabling the window trigger to generate a genuine WAL change. B received zero events for that row, not even a redacted stub, while both authorized `personal` events arrived. Re-run this check against `tccdupkjmwwxcvpqnpeb` once the migrations are deployed there.

  Repeated against hosted Dev (`zihdtxpvugdphieujhzk`) on September 16 with two real authenticated accounts: 14/14 passed. B received the authorized `personal` UPDATE, proving the channel was live, and received nothing for the force-updated locked `for_sn` row. The time gate also rejected a service-role write, which bypasses RLS entirely. One test artifact was found and fixed: firing a change immediately after the `SUBSCRIBED` callback misses it, because `SUBSCRIBED` returns before the `postgres_changes` filter is registered server-side. An earlier run reported a passing no-leak result only because no events were delivered at all, so any future run must assert that an authorized event does arrive.
