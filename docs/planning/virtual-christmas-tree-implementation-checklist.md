...
- **RESOLVED (Sep 16):** That blocker is gone. The import moved to `supabase/seed/`, and `supabase/seed.sql` was repaired (it referenced the dropped `receipt_path` and `draft_*_account` columns and omitted the `NOT NULL` `employee_id`). `supabase db reset --local` now exits 0 with 194/194 migrations applied.
- **VERIFIED (Sep 15):** A two-user local harness signed in as two real authenticated accounts and passed 14/14 assertions covering ownership denial, owner edit, time-gate enforcement, REST redaction, and Realtime redaction.
- **CORRECTION (Sep 15):** Earlier entries in this document described the linked Supabase project as "production." That was wrong. The linked project is `zihdtxpvugdphieujhzk` (**SN Connect [Dev]**, Sydney). Production is `tccdupkjmwwxcvpqnpeb` (**SN Connect [Prod]**, Seoul) and has received none of the Christmas Tree migrations.
- **DEPLOYED TO DEV (Sep 15):** `supabase db push` against the Dev pooler exited 0, applying `20260911000002`, `20260912000001`, `20260915000001`, `20260915000002`, and `20260915000003`.
- **NOT DEPLOYED TO PROD (Sep 15):** superseded by the entry below.
- **DEPLOYED TO PROD (Sep 15):** `tccdupkjmwwxcvpqnpeb` received only the five Christmas migrations, applied in order via `psql` over the Seoul pooler (`aws-1-ap-northeast-2.pooler.supabase.com:6543`) and recorded in `supabase_migrations.schema_migrations`. Applied count moved 170 -> 175. Verified after deployment: 3 tables, 10 policies, 5 triggers, both tables in `supabase_realtime`, `stale_hr_policies=0`, `recursive_subquery=0`, seeded `sn-christmas-wish-tree-2026` active on `Asia/Manila` with October 1 00:00:00 to October 20 23:59:59, and 0 ornament/wish rows.
- **DELIBERATELY NOT DEPLOYED (Sep 15):** Prod is still behind by 21 unrelated migrations, including `20260817133900_import_marketing_ads_data.sql` (381 lines), `20260902113000_import_ea_pending_tasks_dev_v3.sql` (551 lines, DEV assignee), and `20260902113100_import_ea_pending_tasks_prod_v3.sql` (551 lines). A blanket `supabase db push` would have written that data into production as a side effect of shipping this feature, so the Christmas migrations were applied surgically instead. Those 21 need a separate owner decision.
- **CONSEQUENCE:** Because prod migration history now contains later versions while 21 earlier ones are absent, a future `supabase db push` against prod will still try to apply those 21. Resolve them explicitly rather than assuming a push is a no-op.
- **BLOCKED (Sep 15):** Direct migration deployment cannot run from this machine. Outbound TCP 5432 to the Supabase pooler times out, while 6543 and 443 connect. Deploy via the transaction pooler URL on 6543 or from a network that permits 5432.
- **SECURITY (Sep 15):** A Dev database password was entered in plaintext on the command line and persists in shell history and terminal scrollback; rotate it. The Prod password was later reset and supplied only through a masked `Read-Host` prompt, kept in a session variable, and cleared after deployment. Prefer that pattern over inline connection strings.
...
- [x] **1.4** Add RLS policies: authenticated users can read ornaments for the active event, create/update only their own ornament and wishes, and cannot change `user_id`, event, slot, or submission timestamps. Limit event configuration and moderation to approved admin roles. **Effort:** L. **Requirement:** FR-01, FR-04. **Verified locally September 15:** a second signed-in user updating another member's ornament affects 0 rows and raises no server fault. Two defects were found and fixed during verification: `20260915000002` rebuilt policies that referenced the `hr` enum label removed by `20260217000005_consolidate_roles.sql`, and `20260915000003` replaced a self-referencing `WITH CHECK` subquery that made every ornament UPDATE fail with `42P17 infinite recursion detected in policy`. Event/owner immutability now lives in a trigger.
- [x] **1.5 HEADS-UP** Enforce category release and 20th-of-month deadline rules in a database trigger using database/server time, never the browser clock. The UI lock is presentation only; direct writes are rejected before release or after deadline. **Effort:** M. **Requirement:** US-02, FR-04. **Verified locally September 15:** a service-role write, which bypasses RLS entirely, was still rejected with `Wishes for SN are unavailable outside the December submission window`, proving the gate is enforced by the trigger rather than by policy visibility alone.
...
- [x] **2.1** Create Zod-validated routes for event snapshot, ornament placement, and wish upsert. Authenticate every request with the existing server Supabase client. **Effort:** L. **Requirement:** US-01, US-02, US-03. **Completed September 12:** `GET /api/christmas-tree/current`, `POST /api/christmas-tree/ornament`, and `PUT /api/christmas-tree/wishes`.
- [x] **2.2** Implement deterministic collision-aware position allocation: select from server-owned tree-safe zones, reject collisions at the ornament's responsive radius, and return a retryable conflict only when capacity is exhausted. **Effort:** M. **Requirement:** FR-03. **Completed September 12.**
- [x] **2.3** Return a single typed snapshot containing the active event, decoration state, the requesting member's editable wish categories, public ornament summaries, and server-computed release/deadline/teaser labels. Return three ordered October entries only after October opens; never return future-category content. **Effort:** M. **Requirement:** US-02, US-03. **Completed September 12:** snapshot includes owner display names and excludes locked category content.
- [x] **2.4** Add audit-log events for placement and wish submission/edit. Exclude wish content from logs unless retention and access requirements explicitly permit it. **Effort:** M. **Requirement:** FR-01, FR-04. **Completed September 12:** event, ornament, asset, category, and item metadata only; moderation and event-configuration audit events remain with their future admin routes.
...
- [x] **3.1** Build the authenticated Christmas Tree page using existing `@hr-portal/ui` primitives, TanStack Query query-key conventions, loading states, error states, and mutations that refresh the returned snapshot. **Effort:** L. **Requirement:** US-01, US-02. **Completed September 15:** `/christmas-tree` page and query/mutation hooks added.
- [x] **3.2** Build the Christmas-ball selector with all eight approved assets, keyboard navigation, visible selection, name sourced from the authenticated profile, and a clear empty/missing-profile state. **Effort:** M. **Requirement:** US-01, FR-02. **Completed September 15.**
- [x] **3.3** Render the responsive tree using percentage coordinates and stable ornament hit targets; expose owner identity through accessible ornament labels. **Effort:** L. **Requirement:** US-01, FR-03. **Completed September 15.**
- [x] **3.4** Build three server-computed wish categories: three required October personal-wish inputs, one November wish-for-others input, and one December wish-for-SN input. Retain earlier categories, show deadline/submitted state, and hide or disable future categories with exact release labels. **Effort:** M. **Requirement:** US-02. **Completed September 15.**
- [x] **3.5** Render tree progression from server decoration state: add the garland, turn on lights, and illuminate the top star only after the respective team-wide monthly completion threshold. **Effort:** M. **Requirement:** FR-03, FR-04. **Completed September 15.**
- [x] **3.6** Build the ornament detail modal with owner name, ball design, and submitted eligible wishes. Radix Dialog provides pointer, keyboard, focus restoration, and screen-reader semantics. **Effort:** M. **Requirement:** US-03. **Completed September 15.**
...
- [x] **4.1** Add a read-only fullscreen presentation route or mode with minimal chrome, dense readable ornament labels, responsive scaling, exit controls, and no mutation affordances. **Effort:** M. **Requirement:** FR-05. **Completed September 15:** `/christmas-tree` supports a fullscreen presentation mode, with browser-blocked fullscreen falling back to the same read-only view.
- [x] **4.2** Subscribe to allowed ornament and wish changes with the established Supabase Realtime hook pattern; invalidate/refetch the event snapshot and clean up channels on unmount. **Effort:** M. **Requirement:** Optional realtime. **Completed September 15:** subscriptions never render change payloads; they refetch the existing server-redacted snapshot.
- [x] **4.3 HEADS-UP** Confirm Realtime replication and RLS behavior in staging before enabling it in production. A subscription must never receive another user's locked or unauthorized wish payload, even briefly. **Effort:** M. **Requirement:** US-02, US-03. **Verified on hosted Dev and hosted Prod September 16; 14/14 assertions passed in both environments.** `20260915000001` publishes both tables idempotently and pins `REPLICA IDENTITY DEFAULT`. With user B subscribed under its own token via `realtime.setAuth`, a locked `for_sn` row was force-updated by disabling the window trigger to generate a genuine WAL change. B received zero events for that row, not even a redacted stub, while authorized `personal` events arrived.

  Repeated against hosted Dev (`zihdtxpvugdphieujhzk`) on September 16 with two real authenticated accounts: 14/14 passed. B received the authorized `personal` UPDATE, proving the channel was live, and received nothing for the force-updated locked `for_sn` row.

  Repeated against hosted Prod (`tccdupkjmwwxcvpqnpeb`) on September 16 with the same two-user harness and cleanup flow: 14/14 passed (`VERIFY_EXIT=0`). Diagnostic output showed exactly one delivered event (`UPDATE content=PUBLIC_PERSONAL_UPDATED`) and no locked-payload leak after a forced WAL change on the locked row.

  The time gate also rejected a service-role write, which bypasses RLS entirely. One test artifact was found and fixed: firing a change immediately after the `SUBSCRIBED` callback misses it, because `SUBSCRIBED` returns before the `postgres_changes` filter is registered server-side. An earlier run reported a passing no-leak result only because no events were delivered at all, so any future run must assert that an authorized event does arrive.
- [x] **4.4** Provide a polling/refetch fallback and a visible non-intrusive connection state for presentation mode. **Effort:** S. **Requirement:** FR-05. **Completed September 15:** failed Realtime connections trigger 30-second snapshot refreshes and a visible status badge.
