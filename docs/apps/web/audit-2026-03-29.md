...
- Resolution:
  ...
  - The live route pages at `apps/web/src/app/(employee)/dashboard/page.tsx` and `apps/web/src/app/(employee)/associate/dashboard/page.tsx` remain the canonical implementations.
- Follow-up:
...
- Associate dashboard route uses real internship hooks, report submission, and setup redirection.
