...
$pc = "const body = $json.body;`nconst all = Array.isArray(body) ? body : (typeof body === 'string' ? JSON.parse(body) : []);`nconst ctx = $('Set Week Context').first().json;`n`nconst employees = all.filter(e => e.employment_type !== 'associate').sort((a, b) => a.last_name.localeCompare(b.last_name));`nconst interns = all.filter(e => e.employment_type === 'associate').sort((a, b) => a.last_name.localeCompare(b.last_name));`nconst combined = [...employees, ...interns];`n`nreturn [{ json: { employees, interns, combined, employeeCount: combined.length, weekStart: ctx.weekStart, weekLabel: ctx.weekLabel } }];"
