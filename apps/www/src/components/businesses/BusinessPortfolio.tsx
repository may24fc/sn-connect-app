          ...
          {units.map((unit, i) => {
            ...
            return (
              <ScrollReveal key={unit.slug} delay={i * 0.1}>
                <div className="group overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-card transition-all hover:shadow-mega hover:border-zinc-300">
                  {/* Hero image with icon badge */}
                  <Link href={`/businesses/${unit.slug}`} className="relative block h-52 overflow-hidden">
                    <Image
                      src={unit.image}
                      alt={unit.name}
                      fill
                  ...
                  <div className="flex flex-col p-6">
                    <Link href={`/businesses/${unit.slug}`}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {unit.services.slice(0, 3).map((s) => (
                        <span
                          key={s.title}
                          className="rounded-full bg-zinc-50 px-2.5 py-0.5 text-[11px] font-medium text-zinc-600"
                        >
                          {s.title}
                        </span>
                      ))}
                      {unit.services.length > 3 && (
                        <span className="rounded-full bg-zinc-50 px-2.5 py-0.5 text-[11px] font-medium text-zinc-400">
                    ...
                    <div className="mt-4 flex items-center gap-4 border-t border-zinc-100 pt-4">
                      <Link
                        ...
                        className="inline-flex items-center gap-1 text-sm font-semibold text-amber-600 transition-all hover:gap-2"
                      >
                      ...
                      {unit.website_url && (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-sm font-medium text-zinc-400 transition-colors hover:text-amber-600"
                        >
          ...
          {units.map((unit, index) => {
            ...
            return (
              <ScrollReveal key={unit.slug} direction={isEven ? 'left' : 'right'}>
                <div
                  className={cn(
                    'group flex flex-col gap-8 rounded-2xl border border-zinc-200 bg-white p-6 shadow-card transition-all hover:shadow-mega hover:border-zinc-300 sm:p-8 lg:flex-row lg:items-stretch',
                    !isEven && 'lg:flex-row-reverse'
                  )}
                >
                  {/* Hero image */}
                  <div className="relative shrink-0 overflow-hidden rounded-xl min-h-[200px] sm:min-h-[240px] lg:w-80 lg:min-h-0">
                    <Image
                      src={unit.image}
                      alt={unit.name}
                      fill
                  ...
                  <div className="flex flex-1 flex-col justify-center">
                    ...
                    {unit.stats.length > 0 && (
                      <div className="mt-4 flex gap-4">
                        {unit.stats.map((stat) => (
                          <div key={stat.label} className="rounded-lg border border-zinc-100 bg-zinc-50 px-4 py-2">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      {unit.services.length > 3 && (
                        <span className="rounded-full bg-zinc-50 px-3 py-1 text-xs font-medium text-zinc-400">
                          +{unit.services.length - 3} more
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-5 flex items-center gap-4">
                      <Link
                        href={`/businesses/${unit.slug}`}
                        className="inline-flex items-center gap-1 text-sm font-semibold text-amber-600 hover:text-amber-700 transition-colors group-hover:gap-2"
                      >
