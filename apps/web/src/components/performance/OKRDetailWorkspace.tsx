...
const PICKER_METRIC_TYPES = (
  Object.entries(METRIC_TYPE_CONFIG) as Array<[TargetMetricType, (typeof METRIC_TYPE_CONFIG)['number']]>
...
export function OKRDetailWorkspace({ backPath, deleteRedirectPath }: OKRDetailWorkspaceProps): ReactNode {
  ...
  const [objectiveFormState, setObjectiveFormState] = useState<ObjectiveFormState>(emptyObjectiveForm);
  ...
  const { data: evidenceResponse, isLoading: evidenceLoading } = useOKRTargetEvidence(progressTarget?.id);
  ...
  const usedWeight = useMemo(
    () => targets.filter((target) => target.id !== editingTarget?.id).reduce((sum, target) => sum + target.weight, 0),
  ...
  const computedProgress = useMemo(() => {
    ...
    return Math.round(targets.reduce((sum, target) => sum + target.progressPercentage * target.weight, 0) / totalWeight);
  }, [targets]);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0 flex-1">
              <div className="mb-2 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  {okr.description && <p className="mt-0.5 text-sm text-muted-foreground">{okr.description}</p>}
                </div>
              </div>
            ...
            <div className="flex shrink-0 items-center gap-2">
              <div className="relative h-20 w-20">
                <svg className="h-20 w-20 -rotate-90" viewBox="0 0 80 80" aria-labelledby="objective-progress-title" role="img">
              ...
              {okr.status !== 'completed' && (
                <Button variant="outline" size="sm" onClick={() => void handleMarkComplete()} disabled={updateOKR.isPending}>
                  {updateOKR.isPending ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Check className="mr-1 h-4 w-4" />}
                  Complete
                </Button>
              )}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => setObjectiveDeleteOpen(true)} className="text-destructive">
        ...
        <SlidePanelContent size="lg">
          ...
          <SlidePanelBody className="space-y-6">
            <SlidePanelSection label="Objective">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setObjectiveFormState((previous) => ({ ...previous, objective: event.target.value }))
                  }
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setObjectiveFormState((previous) => ({ ...previous, description: event.target.value }))
                  }
            ...
            <SlidePanelSection label="Priority">
              <div className="space-y-1.5">
                <Label htmlFor="objective-weight" className="text-sm font-medium">Weight</Label>
                <div className="relative">
                  <Input
                    ...
                    onChange={(event) =>
                      setObjectiveFormState((previous) => ({ ...previous, weight: event.target.value }))
                    }
                    className="pr-8"
                  />
                  <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">%</span>
                </div>
                <p className={`text-xs ${objectiveWeightInvalid ? 'text-destructive' : 'text-muted-foreground'}`}>
          ...
          <SlidePanelFooter>
            <Button variant="outline" onClick={handleCloseObjectiveEditor}>Cancel</Button>
            <Button onClick={() => void handleSaveObjective()} disabled={!objectiveFormState.objective.trim() || objectiveWeightInvalid || updateOKR.isPending}>
      ...
      <div>
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {targets.length} target{targets.length !== 1 ? 's' : ''} contributing to this objective
            </p>
          </div>
        ...
        {targetsLoading ? (
          <Card>
            <CardContent className="p-6 text-center text-muted-foreground">Loading targets...</CardContent>
          </Card>
        ) : targets.length === 0 ? (
          <Card>
        ...
        <SlidePanelContent size="2xl">
          <SlidePanelHeader>
            ...
            <div className="mt-1 flex items-center gap-1.5">
              <div className={`h-1 flex-1 rounded-full transition-colors ${formStep >= 1 ? 'bg-primary' : 'bg-zinc-200 dark:bg-zinc-700'}`} />
              <span className="text-xs tabular-nums text-muted-foreground">{formStep} / 2</span>
              <div className={`h-1 flex-1 rounded-full transition-colors ${formStep >= 2 ? 'bg-primary' : 'bg-zinc-200 dark:bg-zinc-700'}`} />
            </div>
          </SlidePanelHeader>
            ...
            {formStep === 1 ? (
              <>
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {PICKER_METRIC_TYPES.map(([type, config]) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${isSelected ? 'bg-primary text-primary-foreground' : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400'}`}>
                ...
                <SlidePanelSection label="What are you measuring?">
                  <div className="space-y-1.5">
                    <Label htmlFor="target-name" className="text-sm font-medium">Target Name</Label>
                    <Input
                      id="target-name"
                      placeholder={
                ...
                {formState.metricType !== 'boolean' ? (
                  <SlidePanelSection label="Values">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div className="space-y-1.5">
                        <Label htmlFor="start-value" className="text-sm font-medium">Starting at</Label>
                        <Input id="start-value" type="number" placeholder="0" value={formState.startValue} onChange={(event) => setFormState({ ...formState, startValue: event.target.value })} />
                        <p className="text-xs text-muted-foreground">Where you are now</p>
                      </div>
                      <div className="space-y-1.5">
                        <Label htmlFor="target-value" className="text-sm font-medium">Goal</Label>
                        <Input id="target-value" type="number" placeholder="100" value={formState.targetValue} onChange={(event) => setFormState({ ...formState, targetValue: event.target.value })} />
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div className="space-y-1.5">
                        ...
                        <Select value={formState.unit || 'none'} onValueChange={(value) => setFormState({ ...formState, unit: value === 'none' ? '' : value })}>
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="target-weight" className="text-sm font-medium">Weight</Label>
                        <div className="relative">
                          <Input id="target-weight" type="number" min="1" max={remainingWeight} step="1" placeholder={remainingWeight > 0 ? String(remainingWeight) : '0'} value={formState.weight} onChange={(event) => setFormState({ ...formState, weight: event.target.value })} className="pr-8" />
                          <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">%</span>
                        </div>
                        <p className={`text-xs ${formState.weight !== '' && Number(formState.weight) > remainingWeight ? 'text-destructive' : 'text-muted-foreground'}`}>
                          {remainingWeight <= 0 ? '100% already allocated' : `${remainingWeight}% available`}
                        </p>
                      </div>
                    </div>
                  </SlidePanelSection>
              ...
              <>
                <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900/50">
                  <div className="flex items-center gap-3">
                    <Badge variant="outline" className="shrink-0">{METRIC_TYPE_CONFIG[formState.metricType].label}</Badge>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">{formState.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {formState.metricType === 'boolean'
                          ? 'Done / Not Done'
                    ...
                    return (
                      <div key={level} className="space-y-1.5">
                        <Label htmlFor={`rubric-${level}`} className="text-sm font-medium">
                          Rating {level}{' '}
                          <span className="font-normal text-muted-foreground">— {labels[level - 1]}</span>
                        </Label>
                        <Textarea
                          id={`rubric-${level}`}
                          placeholder={
          ...
          <SlidePanelFooter>
            {formStep === 1 ? (
              <>
                ...
                <Button onClick={() => setFormStep(2)} disabled={!formState.name.trim() || !formState.weight || Number(formState.weight) <= 0 || Number(formState.weight) > remainingWeight}>
                  Next: Define Rubrics
              ...
              <>
                ...
                <Button onClick={() => void handleSaveTarget()} disabled={!formState.rubric1.trim() || !formState.rubric2.trim() || !formState.rubric3.trim() || !formState.rubric4.trim() || createTarget.isPending || updateTarget.isPending}>
        ...
        <DialogContent className="flex max-h-[calc(100vh-2rem)] flex-col overflow-hidden p-0 sm:max-w-[560px]">
              ...
              <div className="space-y-4">
                {progressTarget.metricType === 'boolean' ? (
                  <div className="flex items-center justify-between rounded-lg bg-muted/50 p-4">
                    <span className="font-medium">{progressTarget.currentValue === 0 ? 'Not completed' : 'Completed'}</span>
                    <Button size="sm" variant={progressTarget.currentValue === 0 ? 'default' : 'outline'} onClick={() => void handleUpdateProgress()}>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">
                        Range: {progressTarget.startValue} to {progressTarget.targetValue}
                        {progressTarget.unit ? ` ${progressTarget.unit}` : ''}
                      </span>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="progress-val">Current Value</Label>
                      <Input id="progress-val" type="number" value={progressValue} onChange={(event) => setProgressValue(event.target.value)} />
                    </div>
                    <Progress
                      value={
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {([1, 2, 3, 4] as const).map((level) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-sm font-bold ${isSelected ? 'bg-primary text-primary-foreground' : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400'}`}>
                              {level}
                            </div>
                            <p className="text-sm text-muted-foreground">{rubricText || `Rating ${level}`}</p>
                          </button>
                        );
                      })}
                    </div>
                ...
                <div className="space-y-3 border-t pt-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Attach images or documents that support this progress update.
                    </p>
                  </div>

                  {evidenceLoading ? (
                    <div className="rounded-lg border border-dashed border-border p-3 text-sm text-muted-foreground">Loading evidence...</div>
                  ) : evidenceItems.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-border p-3 text-sm text-muted-foreground">No evidence attached yet.</div>
                  ) : (
                    <div className="space-y-2">
                      {evidenceItems.map((item) => (
                        <div key={item.id} className="flex items-start justify-between gap-3 rounded-lg border border-border p-3">
                          <div className="min-w-0 flex-1 space-y-1">
                            <p className="truncate text-sm font-medium">{item.label || item.file_name || 'Evidence attachment'}</p>
                            <p className="truncate text-xs text-muted-foreground">{item.file_name || item.content}</p>
                            <p className="text-xs text-muted-foreground">
                          ...
                          <div className="flex shrink-0 items-center gap-1">
                            {item.download_url && (
                              <Button type="button" variant="outline" size="sm" onClick={() => window.open(item.download_url || '', '_blank', 'noopener,noreferrer')}>
                                <Download className="mr-1 h-3.5 w-3.5" />
                                View
                              </Button>
                            )}
                            {item.submitted_by === user?.id && (
                              <Button type="button" variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => void handleDeleteEvidence(item.id)} disabled={deleteEvidence.isPending}>
                  ...
                  <div className="space-y-3 rounded-lg border border-border p-3">
                    <div className="space-y-1.5">
                      ...
                      <Input id="evidence-label" value={evidenceLabel} onChange={(event) => setEvidenceLabel(event.target.value)} placeholder="Optional note about this attachment" />
                    </div>
                    ...
                    <FileDropZone
                      ...
                      onRemoveFile={(index) => {
                        setSelectedEvidenceFiles((previous) => previous.filter((_, fileIndex) => fileIndex !== index));
                      }}
                    ...
                    <div className="flex justify-end">
                      <Button type="button" variant="outline" onClick={() => void handleUploadEvidence()} disabled={selectedEvidenceFiles.length === 0 || createEvidence.isPending}>
          ...
          <DialogFooter className="border-t px-6 pb-6 pt-4">
            ...
            {(progressTarget?.metricType !== 'boolean' || progressTarget?.rubric1) && (
              <Button onClick={() => void handleUpdateProgress()} disabled={updateTarget.isPending}>Update</Button>
            )}
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={objectiveDeleteOpen} onOpenChange={setObjectiveDeleteOpen}>
        <DialogContent className="sm:max-w-[420px]">
          ...
          <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-3 text-sm text-muted-foreground">
            <p>
              This action cannot be undone. Reviewers will no longer see this objective once it is deleted.
            </p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setObjectiveDeleteOpen(false)}>Cancel</Button>
            <Button variant="destructive" onClick={() => void handleDeleteObjective()} disabled={deleteOKR.isPending}>
    ...
    <Card className="transition-shadow hover:shadow-sm">
      <CardContent className="p-4">
        <div className="flex items-start gap-4">
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${target.progressPercentage >= 100 ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}>
            {target.progressPercentage >= 100 ? <Check className="h-5 w-5" /> : <Icon className="h-5 w-5" />}
          </div>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                {target.description && (
                  <p className="mt-0.5 line-clamp-1 text-sm text-muted-foreground">{target.description}</p>
                )}
              </div>
            ...
            <div className="mt-3 flex items-center gap-3">
              <div className="flex-1">
                <Progress value={Math.min(target.progressPercentage, 100)} className="h-2" indicatorClassName={getProgressBarColor(target.progressPercentage)} />
              </div>
              <span className={`min-w-[3rem] text-right text-sm font-semibold ${getProgressColor(target.progressPercentage)}`}>
                {target.progressPercentage}%
              </span>
            </div>

            <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
              <Badge variant="outline" className="h-5 text-xs">{config.label}</Badge>
              <span>{displayValue}</span>
              {target.selfRating && <Badge variant="secondary" className="h-5 text-xs">Self: {target.selfRating}/4</Badge>}
              <span className="ml-auto">Weight: {target.weight}%</span>
            </div>

            {target.adminRating && (
              <div className="mt-2 rounded border border-primary/10 bg-primary/5 p-2">
                <p className="text-xs font-medium text-primary">Admin Rating: {target.adminRating}</p>
                {target.adminComments && <p className="mt-0.5 text-xs text-muted-foreground">{target.adminComments}</p>}
              </div>
            )}
          </div>
        </div>
