...
import {
  useGrantRevenueForecastAccess,
  useRevenueForecastAccessGrants,
  useRevokeRevenueForecastAccess,
} from '@/hooks/useRevenueForecastAccess';
import {
  Badge,
  Button,
  Dialog,
...
interface RevenueForecastAccessManagerDialogProps {
  open: boolean;
...
function isMarketingOrFinanceDepartment(department: string | null | undefined): boolean {
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Dialog flow intentionally mirrors existing ATS/CRM access manager UX pattern.
export function RevenueForecastAccessManagerDialog({
  open,
  onOpenChange,
}: RevenueForecastAccessManagerDialogProps) {
  ...
  const grantsQuery = useRevenueForecastAccessGrants(open);
  const grantAccess = useGrantRevenueForecastAccess();
  const revokeAccess = useRevokeRevenueForecastAccess();
  ...
  const candidates = useMemo(() => {
    return (directoryQuery.data?.data ?? []).filter(
      (entry) =>
        isMarketingOrFinanceDepartment(entry.department_name) && !grantedUserIds.has(entry.user_id)
    );
  ...
  async function handleGrant(userId: string, fullName: string) {
    try {
      ...
      addToast({
        variant: 'success',
        title: 'Revenue Forecast access granted',
        description: `${fullName} can now access the SFO Revenue Forecast workspace.`,
      });
    } catch (error) {
      addToast({
        variant: 'error',
        title: 'Failed to grant Revenue Forecast access',
  ...
  async function handleRevoke(userId: string, fullName: string) {
    try {
      ...
      addToast({
        variant: 'default',
        title: 'Revenue Forecast access revoked',
        description: `${fullName} no longer has Revenue Forecast access.`,
      });
    } catch (error) {
      addToast({
        variant: 'error',
        title: 'Failed to revoke Revenue Forecast access',
        description: error instanceof Error ? error.message : 'Please try again.',
      });
    }
  }
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400">
                  These users can view forecasts and log monthly revenue without super-admin
                  elevation.
                </p>
              </div>
            ...
            {isLoading ? (
              ...
              <div className="flex min-h-32 items-center justify-center rounded-lg border border-red-200 bg-red-50 px-4 text-sm text-red-600 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-300">
                ...
                {(directoryQuery.error instanceof Error && directoryQuery.error.message) ||
                  ...
                  'Failed to load Revenue Forecast access data.'}
              </div>
            ) : candidates.length === 0 ? (
              <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-zinc-500 dark:text-zinc-400">
                {search
                      ...
                      <div className="min-w-0 space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                          ...
                          {candidate.department_name ? (
                            <>
                              <span>•</span>
                              <span>{candidate.department_name}</span>
                            </>
                          ) : null}
                        </div>
                      </div>
...
export function RevenueForecastAccessManagerButton() {
  ...
  return (
    <>
      ...
      <RevenueForecastAccessManagerDialog open={open} onOpenChange={setOpen} />
    </>
  );
}
