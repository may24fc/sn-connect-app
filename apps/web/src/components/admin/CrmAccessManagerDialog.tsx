...
interface CrmAccessManagerDialogProps {
  open: boolean;
...
export function CrmAccessManagerDialog({
  open,
  onOpenChange,
  tracker,
  label,
}: CrmAccessManagerDialogProps) {
  ...
  const grantsQuery = useCrmAccessGrants(tracker, open);
  const grantAccess = useGrantCrmAccess(tracker);
  const revokeAccess = useRevokeCrmAccess(tracker);
  ...
  async function handleGrant(userId: string, fullName: string) {
    try {
      ...
      addToast({
        ...
        description: `${fullName} can now access ${label}.`,
      });
    } catch (error) {
      addToast({
        variant: 'error',
        title: 'Failed to grant CRM access',
  ...
  async function handleRevoke(userId: string, fullName: string) {
    try {
      ...
      addToast({
        variant: 'default',
        title: `${label} access revoked`,
        description: `${fullName} no longer has access to ${label}.`,
      });
    } catch (error) {
      addToast({
        variant: 'error',
        title: 'Failed to revoke CRM access',
        description: error instanceof Error ? error.message : 'Please try again.',
      });
    }
  }
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400">
                  These users can access only the {label} tracker.
                </p>
              </div>
            ...
            {grants.length === 0 ? (
              <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-zinc-500 dark:text-zinc-400">
                No delegates for this tracker yet.
              </div>
            ) : (
            ...
            {isLoading ? (
              ...
              <div className="flex min-h-32 items-center justify-center rounded-lg border border-red-200 bg-red-50 px-4 text-sm text-red-600 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-300">
                ...
                {(directoryQuery.error instanceof Error && directoryQuery.error.message) ||
                  ...
                  'Failed to load CRM access data.'}
              </div>
            ) : candidates.length === 0 ? (
              <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-zinc-500 dark:text-zinc-400">
                {search
...
export function CrmAccessManagerButton({ tracker, label }: { tracker: CrmTrackerKey; label: string }) {
  ...
  return (
    <>
      ...
      <CrmAccessManagerDialog open={open} onOpenChange={setOpen} tracker={tracker} label={label} />
    </>
  );
}
