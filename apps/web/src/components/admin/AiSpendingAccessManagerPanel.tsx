...
import {
  useAiSpendingAccessGrants,
  useGrantAiSpendingAccess,
  useRevokeAiSpendingAccess,
} from '@/hooks/useAiSpendingAccess';
import {
  Badge,
  Button,
  Dialog,
...
export function AiSpendingAccessManagerPanel() {
  ...
  const grantsQuery = useAiSpendingAccessGrants(true);
  const grantAccess = useGrantAiSpendingAccess();
  const revokeAccess = useRevokeAiSpendingAccess();
  ...
  async function handleGrant(userId: string, fullName: string) {
    try {
      ...
      addToast({
        variant: 'success',
        title: 'AI Spending access granted',
        description: `${fullName} can now log AI expenses in the AI Spending Tracker.`,
      });
    } catch (error) {
      addToast({
        variant: 'error',
        title: 'Failed to grant AI Spending access',
  ...
  async function handleRevoke(userId: string, fullName: string) {
    try {
      ...
      addToast({
        variant: 'default',
        title: 'AI Spending access revoked',
        description: `${fullName} no longer has AI Spending Tracker access.`,
      });
    } catch (error) {
      addToast({
        variant: 'error',
        title: 'Failed to revoke AI Spending access',
        description: error instanceof Error ? error.message : 'Please try again.',
      });
    }
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              These users can log AI expenses and add subcategories in the AI Spending Tracker.
            </p>
          </div>
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Search active employee and associate accounts, then grant AI Spending Tracker permissions.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-32 items-center justify-center rounded-lg border border-red-200 bg-red-50 px-4 text-sm text-red-600 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-300">
            ...
            {(directoryQuery.error instanceof Error && directoryQuery.error.message) ||
              ...
              'Failed to load AI Spending access data.'}
          </div>
        ) : candidates.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-zinc-500 dark:text-zinc-400">
            {search
              ? 'No matching active employee or associate accounts found.'
              : 'Everyone in the current result set already has AI Spending access.'}
          </div>
        ) : (
          <div className="space-y-2">
      ...
      <div className="rounded-lg border border-border bg-zinc-50 px-4 py-3 text-xs text-zinc-500 dark:bg-zinc-900/40 dark:text-zinc-400">
        <div className="flex items-start gap-2">
          ...
          <p>
            Admins and super admins always have AI Spending Tracker access. Employees and associates need an
            active grant to log expenses and add subcategories.
          </p>
        </div>
      </div>
    </div>
...
interface AiSpendingAccessManagerDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function AiSpendingAccessManagerDialog({
  open,
  onOpenChange,
}: AiSpendingAccessManagerDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-hidden">
        <DialogHeader>
          <DialogTitle>Manage AI Spending access</DialogTitle>
          <DialogDescription>
            Grant active employee or associate users access to the AI Spending Tracker.
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-[calc(85vh-7rem)] overflow-y-auto pr-1">
          <AiSpendingAccessManagerPanel />
        </div>
      </DialogContent>
    </Dialog>
  );
...
export function AiSpendingAccessManagerButton() {
  ...
  return (
    <>
      ...
      <AiSpendingAccessManagerDialog open={open} onOpenChange={setOpen} />
    </>
  );
}
