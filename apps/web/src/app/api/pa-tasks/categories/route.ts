...
const categoryUpdateSchema = paTaskLookupUpdateSchema.extend({
...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('Failed to fetch PA task categories:', error);
      return NextResponse.json({ error: 'Failed to fetch categories' }, { status: 500 });
    }

    return NextResponse.json({ data: data ?? [] });
  } catch (error) {
    console.error('Unexpected error in GET /api/pa-tasks/categories:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error || !data) {
      console.error('Failed to create PA task category:', error);
      return NextResponse.json({ error: getPaTaskWriteErrorMessage(error) }, { status: 500 });
    }

    logActivity(supabaseAdmin, {
      userId: user.id,
      action: 'create_pa_task_category',
      tableName: 'pa_task_categories',
      recordId: data.id,
    ...
    console.error('Unexpected error in POST /api/pa-tasks/categories:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (error || !data) {
      console.error('Failed to update PA task category:', error);
      return NextResponse.json({ error: getPaTaskWriteErrorMessage(error) }, { status: 500 });
    }

    logActivity(supabaseAdmin, {
      userId: user.id,
      action: 'update_pa_task_category',
      tableName: 'pa_task_categories',
      recordId: data.id,
      metadata: { label: data.label },
    });
    ...
    console.error('Unexpected error in PATCH /api/pa-tasks/categories:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const parsed = categoryDeleteSchema.safeParse(await request.json().catch(() => ({})));
    ...
    if (error || !data) {
      console.error('Failed to delete PA task category:', error);
      return NextResponse.json({ error: getPaTaskWriteErrorMessage(error) }, { status: 500 });
    }

    logActivity(supabaseAdmin, {
      userId: user.id,
      action: 'delete_pa_task_category',
      tableName: 'pa_task_categories',
      recordId: id,
    });
    ...
    console.error('Unexpected error in DELETE /api/pa-tasks/categories:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
