...
export function KPICard({ kpi, className, compact = false }: KPICardProps): React.ReactNode {
  ...
  if (compact) {
    return (
      <div className={cn('flex items-center justify-between p-4 rounded-lg border border-border', className)}>
...
export function KPISummary({ kpis, className }: KPISummaryProps): React.ReactNode {
  ...
  const averageScore = totalWeight > 0 ? weightedScore : kpis.reduce((sum, kpi) => sum + kpi.score, 0) / kpis.length;
